...
class BIR(threading.Thread):
	...
	def __init__(self, port,  output_queue, callback, settings, publish_event):
		...
		self.pin = 17
		self.Buttons = [0x300ff22dd, 0x300ffc23d, 0x300ff629d, 0x300ffa857, 0x300ff9867, 0x300ffb04f, 0x300ff6897, 0x300ff02fd,
		...
		while True:
			...
			for button in range(len(self.Buttons)):  # Runs through every value in list
				if hex(self.Buttons[button]) == inData:  # Checks this against incoming
					...
					self.callback(self.ButtonsNames[button], self.settings, self.publish_event)
